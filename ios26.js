...
(function(){
  ...
  function updateMiniBar(card){
    ...
    $('a', mb).href = 'https://wa.me/917439133880?text='+encodeURIComponent('Hi DigiRise! '+n.trim()+' package book karna hai.');
  }
...
(function(){
  ...
  var fg = $('.bt-fg', b), C=2*Math.PI*18, pend=false;
  addEventListener('scroll', function(){
    if(pend) return; pend=true;
    raf(function(){
      var h = document.documentElement.scrollHeight - innerHeight;
...
/* ═════════════ FIX-304b: hero iPhone 15 Pro showcase — 360° drag ═════════════ */
(function(){
  ...
  /* 360° Y-axis drag rotation */
  var ph = $('#heroPhone', zone), rot = 0, sx = null, base = 0;
  ph.addEventListener('pointerdown', function(e){ sx = e.clientX; base = rot; ph.setPointerCapture(e.pointerId); ph.style.transition='none'; });
  ph.addEventListener('pointermove', function(e){
    if(sx==null) return;
    rot = base + (e.clientX - sx) * 0.6;
    ph.style.transform = 'rotateY(' + rot + 'deg)';
  });
  function release(){
    if(sx==null) return; sx=null;
    ph.style.transition='';
    var snapped = Math.round(rot/360)*360;
    rot = snapped; ph.style.transform = 'rotateY(' + snapped + 'deg)';
  }
...
/* ── FIX-320: client logo dual-row marquee + tooltips ── */
(function(){
  var CLIENTS = [
    {img:'assets/lal.png', name:'LAL Sweets', cat:'Sweets & FMCG', tip:'Social system + Meta Ads creatives'},
    {img:'assets/kirtilals.png', name:'Kirtilals', cat:'Luxury Jewellery', tip:'3D Blender visuals + ad creatives'},
    {img:'assets/tradescribe.png', name:'TradeScribe', cat:'B2B / SaaS', tip:'Brand identity + content design'},
    {img:'assets/farmfres.png', name:'FarmFres', cat:'Agro & Food', tip:'Product creatives + packaging design'},
    {img:null, name:'Murzban', cat:'Lifestyle Brand', tip:'Video edits + social content'},
    {img:null, name:'The Liquid Lounge', cat:'Hospitality', tip:'Web + local presence'}
  ];
  function chip(c){
    return '<div class="cm-chip">'
      + (c.img ? '<img src="'+c.img+'" alt="'+c.name+' logo" loading="lazy" onerror="this.remove()">' : '')
...
(function(){
  ...
  var ACTIONS = [
    {ico:'💬', lbl:'WhatsApp', bg:'#25d366', href:'https://wa.me/917439133880?text=Hi%20DigiRise!%20Baat%20karni%20hai!'},
    {ico:'📞', lbl:'Call Now', bg:'linear-gradient(145deg,#f0a825,#b87b0a)', href:'tel:+917439133880'},
