...
(function () {
  ...
  function initNavbar() {
    ...
    if (!nav) return;
    /* FIX-518: Tools Hub nav is a static absolute topbar (tools.css),
       not the floating homepage pill — compact/shadow morphing doesn't
       apply there and previously stacked visual noise on top of the
       sidebar layout. */
    if (document.body.classList.contains('tools-hub-mode')) return;
  ...
  /* ───────────────────────────────────────────
     3 · VELOCITY MARQUEE (scroll-reactive speed)
        PERF FIX: rAF loop now pauses when velocity
        decays to zero and when tab is hidden.
     ─────────────────────────────────────────── */
  function initVelocityMarquee() {
    if (REDUCE) return;
    var tracks = document.querySelectorAll('.marquee-track');
    if (!tracks.length) return;
    var lastY = window.scrollY, vel = 0, pos = 0, rafId = null;
    var running = false;

    window.addEventListener('scroll', function () {
      var delta = Math.min(Math.abs(window.scrollY - lastY), 40);
      vel += delta;
      lastY = window.scrollY;
      if (!running) startLoop(); // only start if not already running
    }, { passive: true });
    ...
    function tick() {
      ...
      // PERF: stop loop when velocity is negligible
      if (vel > 0.05 && !document.hidden) {
  ...
  /* ───────────────────────────────────────────
     5 · 3D TILT — REMOVED (duplicate)
        ios26.js FIX-303 already handles pointer
        tilt on .pkg-card with the CSS-variable
        approach. Running both causes double
        getBoundingClientRect per pointermove.
     ─────────────────────────────────────────── */
  function initTilt() {
    // intentionally empty — handled by ios26.js FIX-303
  }

  /* ───────────────────────────────────────────
     6 · TIMELINE: scroll-drawn line fill
        PERF FIX: scroll listener only active
        when timeline section is near viewport
     ─────────────────────────────────────────── */
  function initTimelineDraw() {
    ...
    // Only mark active when section is near viewport
    new IntersectionObserver(function(entries) {
      active = entries[0].isIntersecting;
      if (active) onScroll(); // immediate update on enter
    }, { rootMargin: '200px 0px 200px 0px' }).observe(sec);
  }

  /* ───────────────────────────────────────────
     7 · PROCESS: travelling flow dot
        PERF FIX: getBoundingClientRect only fires
  ...
  function initFlowDot() {
    ...
    var idx = 0;
    var inView = false;
    // Only run getBoundingClientRect when section is visible
    new IntersectionObserver(function(entries) {
      inView = entries[0].isIntersecting;
    }, {threshold: 0.1}).observe(sec);
    setInterval(function () {
      if (!inView || document.hidden) return; // PERF: skip when off-screen
  ...
  function initFaqSearch() {
    ...
    noRes.innerHTML = 'Kuch nahi mila — <a href="mailto:digiriseindia@gmail.com" target="_blank" rel="noopener" style="color:var(--gold);font-weight:700;">Email pe seedha pucho →</a>';
  ...
  function initWaWidget() {
    var WA = 'mailto:digiriseindia@gmail.com?text=';
  ...
  function initExitIntent() {
    ...
    overlay.innerHTML =
      '<div class="ng-exit-modal" role="dialog" aria-modal="true" aria-label="Free audit offer">' +
        ...
        '<a href="mailto:digiriseindia@gmail.com?subject=Hi!%20Free%20Digital%20Audit%20claim%20karna%20hai%20(exit%20offer)" target="_blank" rel="noopener" class="btn-primary">🎁 Free Audit Claim Karo</a>' +
      '</div>';
  ...
  function initReveals() {
    ...
    els.forEach(function (el) {
      var r = el.getBoundingClientRect();
      if (r.top < _vh * 1.2 && r.bottom > -100) { el.classList.add('ng-in'); return; }
    ...
    els.forEach(function (el) { if(!el.classList.contains('ng-in')) io.observe(el); });
    /* v7.0 watchdog */
    setTimeout(function(){ els.forEach(function(el){ el.classList.add('ng-in'); el.classList.remove('reveal-hidden'); }); }, 1200);
  }
